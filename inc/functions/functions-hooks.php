<?php
function techpro_css() {

	wp_enqueue_style('techpro-style', get_stylesheet_uri());
	...
	if(!empty($options['colors_default'])){
	 ...
	 if ( $custom_styles != '' ) {
	  $css = preg_replace("/(^[\r\n]*|[\r\n]+)[\s\t]*[\r\n]+/", "\n", $custom_styles);
		wp_add_inline_style( 'techpro-style', $css );
	}

}
add_action( 'wp_enqueue_scripts', 'techpro_css');

function techpro_header_script() {
    ...
    function techpro_fonts_url() {
      ...
      $theme_font = "Nunito:200,400,600,700,900";
         ...
         */
        if ( 'off' !== _x( 'on', 'Google font: on or off', 'techpro' ) ) {
            $font_url = add_query_arg( 'family', urlencode( ''. esc_attr($theme_font) .'' ), "//fonts.googleapis.com/css" );
        }
        return $font_url;
    }
    wp_enqueue_style( 'techpro-fonts', techpro_fonts_url(), array(), '1.0.0' );
}
add_action('wp_enqueue_scripts', 'techpro_header_script');




function techpro_admin_script() {
	wp_enqueue_style('techpro-admin', get_template_directory_uri().'/inc/css/admin.css');
}
add_action('admin_enqueue_scripts', 'techpro_admin_script');


add_filter('body_class','techpro_class');
function techpro_class($classes) {
