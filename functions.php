...
function techpro_theme_setup() {
	...
	load_theme_textdomain( 'techpro', get_template_directory() . '/languages' );
	$locale = get_locale();
	...
	register_nav_menus( array(
		'primary' => esc_html__( 'Header Menu', 'techpro' ),
		'mobile' => esc_html__( 'Mobile Menu', 'techpro' ),
		'footer_menu' => esc_html__( 'Footer Navigation', 'techpro' ),
	) );
	if ( ! isset( $content_width ) ) { $content_width = 900; }
}

add_action( 'after_setup_theme', 'techpro_theme_setup' );
...
function techpro_javascript_detection() {
	echo "<script>(function(html){html.className = html.className.replace(/\bno-js\b/,'js')})(document.documentElement);</script>\n";
}
add_action( 'wp_head', 'techpro_javascript_detection', 0 );

if ( ! isset( $content_width ) ) {
	$content_width = 740;
}

function techpro_import_files() {
    return array(
        array(
            'import_file_name'             => esc_html__( 'techpro Full', 'techpro' ),
...
function techpro_after_import_setup() {
...
add_action( 'pt-ocdi/after_import', 'techpro_after_import_setup' );
